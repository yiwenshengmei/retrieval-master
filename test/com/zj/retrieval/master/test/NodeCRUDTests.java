...
public class NodeCRUDTests {
	...
	private void getNode(Session sess) {
		...
		logger.debug("查询到的节点数量: " + nodes.size());
		if (nodes.size() > 0) {
			Node node = (Node) nodes.get(0);
			logger.debug("节点名称: " + node.getName());
			...
			for (NodeAttribute attr : node.getAttributes()) {
				logger.debug("节点属性键: " + attr.getKey());
				logger.debug("节点属性值: " + attr.getValue());
			}
			...
			for (NodeFeature ft : rds.getFeatures()) {
				...
				for (FeatureImage img : ft.getImages()) {
					logger.debug("特征图片路径: " + img.getPath());
				}
			}
		}
	}
