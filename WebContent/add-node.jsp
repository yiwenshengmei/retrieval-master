...
<head>
	...
	function process_new_attr() {
		...
		for (var i = 0; i < new_attr_index; i++) {
			...
			alert('debug[新属性自定义字段key]:' + new_attr_user_field_key);
			alert('debug[新属性自定义字段value]:' + new_attr_user_field_value);
			
			// 拼接出新属性中自定义字段的json字符串
			var new_attr_user_field_json = '[ ';
			for (var j = 0; j < new_attr_user_field_key.length; j++) {
				one_user_filed += '{ ' +
					'key: '   + new_atr_user_field_key[i]    + ', ' +
					'value: ' + new_attr_user_field_value[i] +
					'}';
...
<body>
	<form style="width:700px;margin:0 auto;" id="add_node_form" action="root-node/add" method="post" enctype="multipart/form-data" >
		<table id="base_info">
